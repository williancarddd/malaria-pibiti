...
save_net_name_test = path_project / '6_resultados' / "mobile_test.csv"
save_net_name_test2 = path_project / '6_resultados' / "mobile_testKeras.csv"
save_net_name_train = path_project / '6_resultados' / "mobile_train.csv"
...
def load_dataset(image_paths, folder_working):
    imagens, labels = list(), list()
    for p in image_paths:
        p = os.path.join(folder_working, p)
        imagens.append(p)
        labels.append(p.split('.')[-2][-1])
    return np.asarray(imagens), labels


def load_dataset_part(csv_file, folder_working):
    df = pd.read_csv(csv_file)
    train_df = df[df['Train'] == True]
    test_df = df[df['Test'] == True]
    
    train_X, train_Y = load_dataset(train_df['Image'].values,folder_working)
    test_x, test_y = load_dataset(test_df['Image'].values, folder_working)
...
if __name__ == '__main__':
    ...
    for file in files_parts:
        
            
        csv_logger = tf.keras.callbacks.CSVLogger(save_metrics_path / f"mobile/{file.split('.')[0]}-epoch-results.csv", separator=',', append=True)

        print(f"Step: {file}")
        

        initModel, checkpoint = model_mobilenet()
        train_under_X, train_under_Y, test_under_X, test_under_Y = load_dataset_part(base_path_parts / file, 'Dataset01_100')
        print(train_under_Y.size)
        ...
        predict_test_mobileNet(pred_mobile, test_under_Y, file)

        calculateMeasuresTrain(history_net, file)
        calculateMeasuresTest(history_net, file)
        save_parts_proc(file)
